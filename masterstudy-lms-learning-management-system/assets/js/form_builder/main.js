...
new Vue({
  ...
  methods: {
    ...
    saveForms: function saveForms() {
      ...
      _this.loading = true;
      var data = {
        action: 'stm_lms_save_forms',
        nonce: stm_lms_nonces['stm_lms_save_forms'],
        forms: _this.forms
      };

      _this.$http.post("".concat(stm_lms_ajaxurl, "?action=").concat(data.action, "&nonce=").concat(data.nonce), data.forms).then(function (r) {
        _this.loading = false;
      });
    }
  },
