...
class STM_LMS_Quiz
{
    ...
    public static function init()
    {
        ...
        add_action('h5p_alter_library_styles', function(&$styles) {
			$styles[] = (object) array(
				...
				'path' => STM_LMS_URL . '/assets/css/parts/quiz/h5p.css',
	...
	public static function h5p_results() {
		...
		$last_quiz = STM_LMS_Helpers::simplify_db_array(stm_lms_get_user_last_quiz($user_id, $quiz_id, array('progress', 'status')));

		if(!empty($last_quiz) and !empty($last_quiz['status']) and $last_quiz['status'] === 'passed') wp_send_json($res);

		$status = (!empty($_POST['success'])) ? sanitize_text_field($_POST['success']) : 'failed';

		$status = (!empty($status) and $status === 'true') ? 'passed' : 'failed';
		...
		if(!isset($_POST['success'])) {
			if($progress === 100) $status = 'passed';
		}

		$sequency = '';

		$res['completed'] = ($status === 'passed');
                ...
                case 'fill_the_gap' :
                    ...
                    if (!empty($answers[0]) and !empty($answers[0]['text'])) {
                        ...
                        foreach ($matches as $i => $correct_answer) {
                            $correct = true;
                            if (!isset($answer[$i]) or !isset($correct_answer['answer'])) {
                                $correct = false;
                                break;
                            };
    ...
    public static function get_style($quiz_id) {
    	...
    	if(!empty($quiz_style) && $quiz_style !== 'global') return $quiz_style;
