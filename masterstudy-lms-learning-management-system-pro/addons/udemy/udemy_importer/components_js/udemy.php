<script>
	<?php
	ob_start();
	include STM_LMS_PRO_PATH . '/addons/udemy/udemy_importer/components/udemy.php';
		...
		methods: {
			search: function () {
				...
				this.timer = setTimeout(function () {
					_this.$http.get(stm_wpcfto_ajaxurl + '?action=stm_lms_pro_search_courses&nonce=' + stm_lms_pro_nonces['stm_lms_pro_search_courses'] + '&s=' + _this.search_name).then(function (r) {
						_this.loading = false;
			...
			importCourse: function (course_id, index) {
				...
				_this.$set(_this.courses[index], 'loading_text', '<?php esc_html_e('Importing Course', 'masterstudy-lms-learning-management-system'); ?>');
